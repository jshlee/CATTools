...
private:
  ...
  int b_nVtx, b_nJet, b_hlt_80_pass, b_hlt_140_pass, b_hlt_320_pass, b_hlt_400_pass, b_hlt_450_pass, b_hlt_500_pass;
...
ColorCoherenceAnalyzer::ColorCoherenceAnalyzer(const edm::ParameterSet& iConfig) :
  ...
  vtxToken_(iConfig.getParameter<edm::InputTag>("vtx"))
{
...
void ColorCoherenceAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  edm::Handle<int> vtx;
  iEvent.getByLabel(vtxToken_, vtx);
...
{
  vector<simpleJet> sort_result;
  map<float, simpleJet> jetList;
  simpleJet sJet;
  for (auto jet : seljets){
    sJet = sysJet(jet, sys_name);
    if (sJet.Pt() > 30.0) jetList[1.0/sJet.Pt()] = sJet;
  }
  map<float, simpleJet>::iterator iter;
  for (iter = jetList.begin(); iter != jetList.end(); iter++){
    sort_result.push_back((*iter).second);
  }
  return sort_result;
}
...
vector<float> ColorCoherenceAnalyzer::calBeta(vector<simpleJet> seljets)
{
  ...
  float del_eta = copysign(seljets[2].Eta() - seljets[1].Eta(), seljets[1].Eta());
...
{
  b_nVtx = -99; b_nJet = -99; b_hlt_80_pass = -99; b_hlt_140_pass = -99;  b_hlt_320_pass = -99; b_hlt_400_pass = -99; b_hlt_450_pass = -99; b_hlt_500_pass = -99;
